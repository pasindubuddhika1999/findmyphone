...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.status(201).json({
        message:
          "Shop registration submitted successfully. Please wait for admin approval before logging in.",
        user: {
          id: user._id,
          username: user.username,
          email: user.email,
      ...
      if (user.role === "shop") {
        ...
        if (!shop.isApproved) {
          return res.status(403).json({
            message:
              "Your shop account is pending approval. Please wait for admin approval before logging in.",
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.role !== "shop") {
        return res
          .status(403)
          .json({ message: "Only shop accounts can update shop profiles" });
      }
      ...
      const updatedShop = await Shop.findByIdAndUpdate(shop._id, updates, {
        new: true,
        runValidators: true,
      });
